...
class CRF(nn.Module):
    ...
    def log_prob(self, sentence: Sentence, corpus: TaggedCorpus) -> TensorType[()]:
        ...
        n = len(sent)
        joint_prob = -float("Inf")

        for i in range (1,n):
            (prev_word, prev_tag) = sent[i-1]
            (curr_word, curr_tag) = sent[i]
            joint_prob += self.A[prev_tag][curr_tag] + self.B[curr_tag][curr_word]

        return joint_prob/self.log_forward(sentence, corpus)
